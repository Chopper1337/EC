...
static void has_target_event(QWORD local_player, QWORD target_player, float fov,
                             vec2 aim_punch, vec3 aimbot_angle, vec2 view_angle,
                             vec3 bone);

static vec3 get_target_angle(QWORD local_player, vec3 position, DWORD num_shots,
                             vec2 aim_punch);
static void get_best_target(BOOL ffa, QWORD local_controller,
                            QWORD local_player, DWORD num_shots, vec2 aim_punch,
                            QWORD *target);
static void standalone_rcs(DWORD shots_fired, vec2 vec_punch,
                           float sensitivity);
...
//
static void cs2::features::has_target_event(QWORD local_player,
                                            QWORD target_player, float fov,
                                            vec2 aim_punch, vec3 aimbot_angle,
                                            vec2 view_angle, vec3 bone) {
#ifndef __linux__
  ...
  if (b_triggerbot_button && config::aimbot_enabled && mouse_down_ms == 0 &&
    ...
    if (aim_punch.x > accurate_shots_fl) {
      ...
      if (math::vec_min_max(eye, dir, math::vec_transform(coll.min, matrix),
...
void cs2::features::run(void) {
  ...
  b_aimbot_button =
      cs2::input::is_button_down(config::aimbot_button) | b_triggerbot_button;
  ...
  float sensitivity = cs2::mouse::get_sensitivity() *
                      cs2::player::get_fov_multipler(local_player);
  ...
  if (config::visuals_enabled == 2) {
    get_best_target(ffa, local_player_controller, local_player, num_shots,
                    aim_punch, &best_target);
  } else {
    //
  ...
  if (aimbot_target == 0) {
    ...
    if (!cs2::player::is_valid(aimbot_target,
                               cs2::player::get_node(aimbot_target))) {
      aimbot_target = best_target;

      if (aimbot_target == 0) {
        aimbot_bone = 0;
        get_best_target(ffa, local_player_controller, local_player, num_shots,
                        aim_punch, &aimbot_target);
      }
    }
  }
  ...
  if (event_state == 0) {
    if (aimbot_fov != 360.0f) {
      features::has_target_event(local_player, aimbot_target, aimbot_fov,
                                 aim_punch, aimbot_angle, view_angle,
                                 aimbot_pos);
    }
  }
...
static void cs2::features::get_best_target(BOOL ffa, QWORD local_controller,
                                           QWORD local_player, DWORD num_shots,
                                           vec2 aim_punch, QWORD *target) {
  ...
  if (best_fov != 360.0f) {
    event_state = 1;
    features::has_target_event(local_player, *target, best_fov, aim_punch,
                               angle, va, aimpos);
  }
}

static void cs2::features::standalone_rcs(DWORD num_shots, vec2 vec_punch,
                                          float sensitivity) {
  if (num_shots > 1) {
...
static void cs2::features::esp(QWORD local_player, QWORD target_player,
                               vec3 head) {
  ...
  if (!math::world_to_screen(screen_size, bottom, screen_bottom, view_matrix) ||
!math::world_to_screen(screen_size, top, screen_top, view_matrix))
  {
          return;
  }
  ...
  float target_health = ((float)cs2::player::get_health(target_player) / 100.0f)
* 255.0f; float r = 255.0f - target_health; float g = target_health; float b =
0.00f;

#ifdef __linux__
  client::DrawFillRect((void *)0, x, y, w, h, (unsigned char)r, (unsigned
char)g, (unsigned char)b); #else QWORD sdl_window_data =
cs2::sdl::get_window_data(sdl_window); if (sdl_window_data == 0) return; QWORD
hwnd = cs2::sdl::get_hwnd(sdl_window_data); client::DrawFillRect((void *)hwnd,
x, y, w, h, (unsigned char)r, (unsigned char)g, (unsigned char)b); #endif
  */
  ...
  if (!math::world_to_screen(screen_size, origin, screen_bottom, view_matrix) ||
      !math::world_to_screen(screen_size, top_origin, screen_top,
                             view_matrix)) {
    return;
  }

  float target_health =
      ((float)cs2::player::get_health(target_player) / 100.0f) * 255.0f;
...
#ifdef __linux__
  client::DrawRect((void *)0, x, y, box_width, box_height, (unsigned char)r,
  ...
  client::DrawRect((void *)hwnd, x, y, box_width, box_height, (unsigned char)r,
                   (unsigned char)g, (unsigned char)b);
#endif
}
