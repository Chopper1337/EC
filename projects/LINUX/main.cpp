...
static ssize_t send_input(int dev, __u16 type, __u16 code, __s32 value) {
...
void mouse1_down(void) { send_input(fd, EV_KEY, 0x110, 1); }

void mouse1_up(void) { send_input(fd, EV_KEY, 0x110, 0); }

void DrawRect(void *hwnd, int x, int y, int w, int h, unsigned char r,
...
void DrawFillRect(void *hwnd, int x, int y, int w, int h, unsigned char r,
                  unsigned char g, unsigned char b) {
  SDL_FRect rect{};
  rect.x = (float)x;
  rect.y = (float)y;
...
int main(void) {
  ...
  SDL_Window *temp =
      SDL_CreateWindow("EC", 1, 1, SDL_WINDOW_MINIMIZED | SDL_WINDOW_OPENGL);
  if (temp == NULL) {
    return 0;
  }
  ...
  SDL_Window *window = SDL_CreatePopupWindow(
      temp, 0, 0, 640, 480,
      SDL_WINDOW_TRANSPARENT | SDL_WINDOW_BORDERLESS | SDL_WINDOW_TOOLTIP);

  SDL_SetWindowSize(window, display_max_x - display_min_x,
